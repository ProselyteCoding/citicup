...
def upload_portfolio():
    ...
    try:
        ...
        # 不再进行计算，直接返回成功
        return (
            jsonify({"success": True, "message": "持仓数据上传成功"}),
            200,
        )
        ...
        return (
            jsonify(
                {"success": False, "message": f"服务器处理数据时发生错误: {str(error)}"}
            ),
            500,
        )
...
def get_hedging_advice_controller():
    ...
    try:
        ...
        # 调用风险策略函数获取风险信息
        try:
            from services.ai_service import qushi_Risk_strategy

            risk_info = qushi_Risk_strategy(current_portfolio)
            ...
            risk_info = {
                "currencyExposure": [
                    {"currency": "EUR/USD", "riskRate": "高风险", "tendency": "上"},
                    {"currency": "USD/JPY", "riskRate": "高风险", "tendency": "下"},
                ],
                ...
                "macroRiskCoefficients": [
                    {"month": 1, "all": 80, "economy": 60, "policy": 40, "market": 20}
                ],
                "riskSignalAnalysis": {
                    "current": {
                        "credit": 60,
                ...
                "singleCurrencyAnalysis": [
                    {"currency": "EUR/USD", "upper": 1.0063, "lower": 0.9938},
                    {"currency": "USD/JPY", "upper": 1.0043, "lower": 0.9958},
                ],
            }
        ...
        combined_data["currentHedgingAdvice"] = hedging_advice.get(
            "currentHedgingAdvice"
        )
        combined_data["positionRiskAssessment"] = hedging_advice.get(
        ...
        combined_data["recommendedPositions"] = hedging_advice.get(
        ...
        combined_data["singleCurrencyAnalysis"] = risk_info.get(
            "singleCurrencyAnalysis"
        )
